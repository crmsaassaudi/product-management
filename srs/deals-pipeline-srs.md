...
- **`BR-21.3` (Ảnh chụp tính vào báo cáo theo trạng thái hiệu lực tại thời điểm chốt báo cáo, không đếm trùng qua các kỳ lồng nhau):** Một báo cáo tổng hợp cho một kỳ cụ thể — hiệu suất người phụ trách (`FEAT-26`), hạn ngạch doanh số (`FEAT-27`), vận tốc bán hàng tính từ thời điểm đóng (`BR-14.1`) — chỉ tính ảnh chụp **đang có hiệu lực tại thời điểm kỳ đó kết thúc**. Không áp dụng cho dự báo doanh thu (`FEAT-25`), vì dự báo chỉ tính cơ hội đang mở và không bao giờ đọc ảnh chụp của cơ hội đã đóng. Áp dụng cụ thể theo hai tình huống:
  - **Kỳ gốc đã kết thúc trước thời điểm mở lại:** báo cáo của kỳ gốc đã kết thúc là bất biến — không được viết lại chỉ vì có một lượt tái phân loại xảy ra sau đó. Báo cáo đó tiếp tục tính ảnh chụp cũ (nay đã Hết hiệu lực) đúng như khi kỳ đó vừa kết thúc. Ảnh chụp mới (từ bước đóng lại) thuộc về kỳ chứa thời điểm đóng lại, không liên quan tới kỳ gốc đã kết thúc.
  - **Kỳ gốc chưa kết thúc tại thời điểm mở lại** (ví dụ đóng nhầm rồi sửa lại trong cùng ngày hoặc cùng kỳ): kỳ đó chưa có báo cáo bất biến nào cần bảo vệ, nên khi kỳ gốc kết thúc, báo cáo của kỳ đó chỉ tính ảnh chụp **đang có hiệu lực tại đúng thời điểm kỳ kết thúc** — nếu cơ hội đã được đóng lại (bước 2) trước khi kỳ gốc kết thúc, báo cáo tính ảnh chụp mới, không tính ảnh chụp cũ đã Hết hiệu lực; nếu cơ hội vẫn đang mở khi kỳ gốc kết thúc, báo cáo của kỳ đó không tính cơ hội này ở cả hai ảnh chụp.
  - **Kỳ lồng nhau (tháng nằm trong quý):** mỗi kỳ, dù là tháng hay quý, áp dụng đúng nguyên tắc trên một cách độc lập theo thời điểm kết thúc của chính kỳ đó — không suy ra kết quả của kỳ lớn hơn bằng cách cộng gộp máy móc các kỳ nhỏ hơn đã tính riêng.
  - **Tái phân loại sang một kỳ đã kết thúc khác (ví dụ kỳ gốc và kỳ đóng lại nằm ở hai quý khác nhau, cả hai đã kết thúc):** đây là tình huống duy nhất mà quy tắc "mỗi kỳ tự bảo vệ báo cáo của chính nó" chưa đủ — cùng một cơ hội có ảnh chụp hợp lệ ở **hai kỳ đã kết thúc khác nhau** (kỳ gốc giữ ảnh chụp Hết hiệu lực, kỳ đóng lại giữ ảnh chụp mới). Với báo cáo của **từng kỳ riêng lẻ**, cả hai đều đúng theo nguyên tắc bất biến ở trên. Nhưng với bất kỳ báo cáo nào **tổng hợp nhiều kỳ đã kết thúc** (ví dụ báo cáo cả năm cộng dồn nhiều quý, hoặc báo cáo lũy kế của một người phụ trách), hệ thống bắt buộc **khử trùng theo cơ hội**: chỉ tính đúng một ảnh chụp cho mỗi cơ hội — ảnh chụp có thời điểm đóng **gần nhất** trong số các ảnh chụp còn tính vào phạm vi báo cáo đó — không cộng cả ảnh chụp Hết hiệu lực lẫn ảnh chụp mới của cùng một cơ hội vào cùng một báo cáo tổng hợp nhiều kỳ.

  **Lý do nghiệp vụ:** Không phân biệt hai tình huống đầu sẽ dẫn tới hai lỗi đối lập: nếu luôn giữ ảnh chụp cũ trong kỳ gốc bất kể kỳ đó đã kết thúc hay chưa, một lượt sửa lỗi nhập liệu trong cùng ngày sẽ khiến cơ hội bị đếm hai lần trong cùng một kỳ chưa kết thúc; nếu luôn loại ảnh chụp cũ ngay khi mở lại bất kể kỳ đã kết thúc hay chưa, một báo cáo quý đã công bố và dùng để chi lương thưởng sẽ bị viết lại chỉ vì một tái phân loại diễn ra ở quý sau. Riêng tình huống tái phân loại giữa hai kỳ đã kết thúc cần thêm quy tắc khử trùng, vì bảo vệ từng kỳ riêng lẻ (đúng) không tự động bảo vệ được báo cáo tổng nhiều kỳ (sai nếu không khử trùng) — nếu không, một hợp đồng Thắng ở Quý 1 rồi tái phân loại và đóng Thắng lại ở Quý 2 sẽ được tính hoa hồng và hạn ngạch **hai lần** trong báo cáo năm, dù mỗi báo cáo quý riêng lẻ đều đúng.
...
| `AC-21.2.3` | Tiếp nối AC-21.2.1 | Mở báo cáo hạn ngạch cả năm (tổng hợp Quý 1 và Quý 2) | Cơ hội chỉ được khử trùng tính đúng một lần theo ảnh chụp có thời điểm đóng gần nhất (kết quả Thua ở Quý 2) — không cộng cả 70%/30% của kết quả Thắng cũ lẫn kết quả Thua mới vào tổng năm |
| `AC-21.2.4` | Cơ hội đóng Thắng ở Quý 1 với giá trị 1 tỷ | Sang Quý 2, mở lại và đóng lại Thắng với giá trị đã sửa thành 1,2 tỷ | Báo cáo hạn ngạch cả năm chỉ tính 1,2 tỷ cho cơ hội này (khử trùng theo ảnh chụp gần nhất) — không cộng thêm 1 tỷ của ảnh chụp Quý 1 đã Hết hiệu lực, tránh ghi công hai lần cho cùng một hợp đồng |
