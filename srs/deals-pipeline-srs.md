...
- **`BR-04.3` (Xóa vĩnh viễn có cascade rõ ràng):** Khi hết thời hạn lưu trữ, cơ hội bị xóa vĩnh viễn theo đúng một quy tắc cho từng loại dữ liệu liên quan: Vé hỗ trợ liên kết được gỡ liên kết nhưng **không** bị xóa theo; Công việc liên kết được gỡ liên kết; các bản ghi trên dòng thời gian hoạt động bị xóa cùng cơ hội; nhật ký kiểm toán liên quan **được giữ lại vĩnh viễn** bất kể cơ hội đã bị xóa; **mọi ảnh chụp kết quả đã từng gắn với cơ hội này (`BR-27.5`), kể cả đang ở trạng thái Hết hiệu lực, cũng được giữ lại vĩnh viễn** cùng cơ chế với nhật ký kiểm toán — vẫn tra cứu được và vẫn tính vào báo cáo của đúng kỳ chứa thời điểm đóng của nó, không bị ảnh hưởng bởi việc bản thân cơ hội đã bị xóa vĩnh viễn.

  **Lý do nghiệp vụ:** Vé hỗ trợ và Công việc là bản ghi độc lập có vòng đời riêng của khách hàng — xóa cơ hội không có nghĩa là các tương tác đó chưa từng xảy ra. Nhật ký kiểm toán phải sống lâu hơn bản ghi mà nó ghi nhận, nếu không việc điều tra "ai đã xóa cơ hội này" sẽ tự mất bằng chứng ngay khi cơ hội biến mất hoàn toàn. Ảnh chụp kết quả chịu đúng lý do đó và thêm một lý do nữa: đây là căn cứ đã dùng để chi hoa hồng và tính hạn ngạch của một kỳ có thể đã khóa sổ (`BR-21.3`) — nếu ảnh chụp mất theo khi xóa cơ hội, một người có quyền tái phân loại và quyền xóa có thể xóa sạch dấu vết của một kết quả tài chính đã công bố chỉ bằng cách mở lại rồi xóa, mà không cần bất kỳ thẩm quyền đảo ngược báo cáo nào khác.
...
| `AC-04.1.5` | Cơ hội đã đóng Thắng ở Quý 1 (kỳ đã kết thúc, đã chi hoa hồng), được tái phân loại về đang mở ở Quý 2 | Chủ sở hữu Không gian làm việc cố gắng xóa cơ hội (nay đang ở trạng thái đang mở) | Từ chối — cơ hội này đã từng mang ảnh chụp kết quả (nay Hết hiệu lực), nên vẫn không xóa được dù đang ở trạng thái đang mở |
...
- **`BR-21.3` (Một khoảng báo cáo bất kỳ chỉ tính đúng một ảnh chụp cho mỗi cơ hội):** Áp dụng cho mọi báo cáo đọc kết quả cơ hội đã đóng theo một khoảng thời gian — hiệu suất người phụ trách (`FEAT-26`), hạn ngạch doanh số (`FEAT-27`), vận tốc bán hàng (`BR-14.1`) — bất kể khoảng đó là một kỳ chuẩn (tháng/quý/năm tài chính theo `BR-27.1`), nhiều kỳ chuẩn cộng lại (ví dụ báo cáo năm gồm nhiều quý), hay một khoảng tùy ý không theo ranh giới kỳ chuẩn (ví dụ "30 ngày gần nhất"). Không áp dụng cho dự báo doanh thu (`FEAT-25`), vì dự báo chỉ tính cơ hội đang mở và không bao giờ đọc ảnh chụp của cơ hội đã đóng.
  ...
  **Quy tắc duy nhất, áp dụng thống nhất cho mọi khoảng báo cáo:** với mỗi cơ hội, báo cáo chỉ tính **đúng một ảnh chụp** — ảnh chụp đang có hiệu lực tại **thời điểm cuối khoảng báo cáo** (hoặc thời điểm xem báo cáo, nếu khoảng đó chưa kết thúc), **và** có **thời điểm đóng nằm trong chính khoảng báo cáo đó**. Nếu ảnh chụp đang có hiệu lực tại thời điểm cuối khoảng có thời điểm đóng nằm **ngoài** khoảng báo cáo (ví dụ cơ hội đóng ở Quý 1, tái phân loại và đóng lại ở Quý 3, nhưng đang xem báo cáo Quý 1+Quý 2), báo cáo đó **không tính cơ hội này** — không lấy tạm ảnh chụp cũ đã Hết hiệu lực, cũng không lấy ảnh chụp mới nằm ngoài khoảng.

  Hệ quả trực tiếp của quy tắc duy nhất này:
  - **Kỳ đã kết thúc là bất biến:** báo cáo của một kỳ đã kết thúc không đổi khi có tái phân loại xảy ra sau đó, vì ảnh chụp có hiệu lực tại thời điểm kỳ đó kết thúc không đổi.
  ...
  **Lý do nghiệp vụ:** Một quy tắc duy nhất áp dụng cho mọi độ dài khoảng thời gian tránh được việc phải định nghĩa riêng "kỳ chuẩn" khác "nhiều kỳ" khác "khoảng tùy ý" — ba định nghĩa riêng rẽ luôn có nguy cơ cho ra ba kết quả khác nhau trên cùng một tình huống, đúng loại mâu thuẫn đã xảy ra khi tài liệu từng tách "kỳ lồng nhau" và "khử trùng nhiều kỳ" thành hai quy tắc riêng. Không có quy tắc "thời điểm đóng phải nằm trong khoảng báo cáo", một cơ hội Thắng ở Quý 1 rồi tái phân loại và đóng lại ở Quý 3 có thể vừa bị tính vào báo cáo Quý 1+Quý 2 (sai, vì kết quả thật nằm ở Quý 3) vừa bị tính lại ở báo cáo năm (đúng một lần) — quy tắc trên đảm bảo báo cáo Quý 1+Quý 2 không tính cơ hội này, tránh đếm trùng ở bất kỳ tổ hợp khoảng thời gian nào.
...
| `AC-21.2.6` | Cơ hội đóng Thắng ở Quý 1. Mở lại ở Quý 2, đóng lại Thắng ở Quý 3 | Mở báo cáo hạn ngạch tổng hợp chỉ Quý 1 và Quý 2 (chưa gồm Quý 3) | Ảnh chụp có hiệu lực tại cuối Quý 2 là ảnh chụp Hết hiệu lực (cơ hội đang mở, chưa đóng lại) — báo cáo Quý 1+Quý 2 không tính cơ hội này ở bất kỳ ảnh chụp nào, không lấy tạm kết quả Thắng cũ đã hết hiệu lực; kết quả thật (đóng lại ở Quý 3) chỉ xuất hiện khi báo cáo mở rộng bao gồm Quý 3 |
...
- **`BR-26.1` (Báo cáo theo người phụ trách, hai chỉ số giá trị tách biệt):** Tổng hợp theo từng người phụ trách trong một kỳ — cơ hội đang mở xét theo ngày dự kiến đóng rơi trong kỳ, cơ hội đã đóng xét theo thời điểm thắng/thua trong ảnh chụp (`BR-27.5`) rơi trong kỳ, nhất quán với cách xác định kỳ tại `BR-27.2`. Báo cáo hiển thị **hai chỉ số giá trị riêng biệt**, không gộp làm một:
