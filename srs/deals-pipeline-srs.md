...
| **Ảnh chụp Kết quả** | Bộ dữ liệu được chốt cứng tại thời điểm một cơ hội đóng — Thắng hoặc Thua — gồm giá trị, mã tiền tệ, giá trị quy đổi, tỷ giá, người phụ trách, đơn vị tổ chức, tỷ lệ chia doanh số, thời điểm đóng, và với nhánh Thua là lý do thất bại. Là nguồn duy nhất cho mọi báo cáo đọc kết quả của cơ hội đã đóng (`BR-27.5`). Khi cơ hội được tái phân loại, ảnh chụp gốc chuyển sang trạng thái **Hết hiệu lực** — không xóa, vẫn tra cứu được, nhưng thôi tham gia báo cáo theo quy tắc tại `BR-21.3`. |
| **Đồng tiền Cơ sở** | Một mã tiền tệ duy nhất do mỗi Không gian làm việc khai báo, dùng làm mẫu số quy đổi cho mọi báo cáo tổng hợp nhiều cơ hội khác loại tiền tệ (`BR-02.2`). |
...
| `AC-04.1.2` | Cơ hội đã đóng Thắng | Chủ sở hữu Không gian làm việc cố gắng xóa | Từ chối: "Cơ hội đã có kết quả đóng, không xóa được" |
...
| `AC-04.1.4` | Cơ hội đã đóng Thua | Cố gắng xóa | Từ chối: "Cơ hội đã có kết quả đóng, không xóa được" — cùng quy tắc với cơ hội đã Thắng |
| `AC-04.1.5` | Cơ hội đã đóng Thắng ở Quý 1 (kỳ đã kết thúc, đã chi hoa hồng), được tái phân loại về đang mở ở Quý 2 | Chủ sở hữu Không gian làm việc cố gắng xóa cơ hội (nay đang ở trạng thái đang mở) | Từ chối: "Cơ hội đã có kết quả đóng, không xóa được" — cơ hội này đã từng mang ảnh chụp kết quả (nay Hết hiệu lực), nên vẫn không xóa được dù đang ở trạng thái đang mở |
| `AC-04.1.6` | Cơ hội đang mở, chưa từng đóng lần nào | Người có quyền xóa cố gắng xóa | Cho phép — cơ hội chưa từng mang ảnh chụp kết quả nào |
...
- **`BR-21.3` (Một khoảng báo cáo bất kỳ chỉ tính đúng một ảnh chụp cho mỗi cơ hội):** Áp dụng cho mọi báo cáo đọc kết quả cơ hội đã đóng theo một khoảng thời gian — hiệu suất người phụ trách (`FEAT-26`), hạn ngạch doanh số (`FEAT-27`), vận tốc bán hàng (`BR-14.1`) — bất kể khoảng đó là một kỳ chuẩn (tháng/quý/năm tài chính theo `BR-27.1`), nhiều kỳ chuẩn cộng lại (ví dụ báo cáo năm gồm nhiều quý), hay một khoảng tùy ý không theo ranh giới kỳ chuẩn (ví dụ "30 ngày gần nhất"). Không áp dụng cho dự báo doanh thu (`FEAT-25`), vì dự báo chỉ tính cơ hội đang mở và không bao giờ đọc ảnh chụp của cơ hội đã đóng.

  **Quy tắc duy nhất, áp dụng thống nhất cho mọi khoảng báo cáo:** với mỗi cơ hội, báo cáo chỉ tính **đúng một ảnh chụp** — ảnh chụp đang có hiệu lực tại **thời điểm cuối khoảng báo cáo** (hoặc thời điểm xem báo cáo, nếu khoảng đó chưa kết thúc), **và** có **thời điểm đóng nằm trong chính khoảng báo cáo đó**. Vì một ảnh chụp chỉ có hiệu lực từ đúng thời điểm nó được chốt trở đi, "đang có hiệu lực tại cuối khoảng" tự nó đã kéo theo "thời điểm đóng không muộn hơn cuối khoảng" — điều kiện thứ hai chỉ còn tác dụng khi thời điểm đóng đó **sớm hơn cả lúc khoảng báo cáo bắt đầu** (ví dụ khoảng báo cáo là Quý 2, nhưng cơ hội đóng và chưa từng bị tái phân loại lại kể từ Quý 4 năm trước — khoảng báo cáo Quý 2 không tính cơ hội này, vì nó không đóng trong quý đó và cũng chẳng còn gì để "tính lại"). Nếu cơ hội vẫn đang mở tại thời điểm cuối khoảng (chưa có ảnh chụp nào đang hiệu lực, dù trước đó đã có một ảnh chụp Hết hiệu lực), báo cáo **không tính cơ hội này** — không lấy tạm ảnh chụp cũ đã Hết hiệu lực.

  Hệ quả trực tiếp của quy tắc duy nhất này:
  - **Kỳ đã kết thúc là bất biến:** báo cáo của một kỳ đã kết thúc không đổi khi có tái phân loại xảy ra sau đó — dù tái phân loại đó xảy ra ở cùng kỳ đang xem hay một kỳ khác, ảnh chụp có hiệu lực tại thời điểm kỳ đó kết thúc không đổi. Ví dụ: cơ hội đóng Thắng ở Quý 1, sau đó tái phân loại và đóng lại Thua ở Quý 3 — báo cáo Quý 1 (xem riêng) và báo cáo Quý 1+Quý 2 (xem gộp, trước khi tới Quý 3) đều vẫn tính đúng kết quả Thắng cũ, vì tại thời điểm cuối Quý 2, ảnh chụp Thắng vẫn đang có hiệu lực và đóng trong đúng khoảng đang xét.
  - **Tái phân loại trong cùng kỳ chưa kết thúc:** báo cáo của kỳ đó, khi kết thúc, tính đúng ảnh chụp có hiệu lực lúc đó (ảnh chụp mới nếu đã đóng lại, hoặc không tính cơ hội này nếu vẫn đang mở) — không đếm trùng cả ảnh chụp cũ lẫn mới.
  - **Kỳ lớn hơn không suy ra được bằng cách cộng các kỳ nhỏ hơn đã tính riêng:** báo cáo của kỳ lớn (ví dụ năm) áp dụng lại đúng quy tắc duy nhất trên chính khoảng của kỳ lớn đó, không cộng dồn kết quả của các kỳ nhỏ (tháng/quý) đã tính riêng lẻ — vì thời điểm đóng có hiệu lực tại cuối kỳ lớn có thể khác thời điểm đóng có hiệu lực tại cuối từng kỳ nhỏ bên trong nó.

  **Lý do nghiệp vụ:** Một quy tắc duy nhất áp dụng cho mọi độ dài khoảng thời gian tránh được việc phải định nghĩa riêng "kỳ chuẩn" khác "nhiều kỳ" khác "khoảng tùy ý" — ba định nghĩa riêng rẽ luôn có nguy cơ cho ra ba kết quả khác nhau trên cùng một tình huống, đúng loại mâu thuẫn đã từng xảy ra khi tài liệu tách "kỳ lồng nhau" và "khử trùng nhiều kỳ" thành hai quy tắc riêng biệt: hai quy tắc đó cho hai đáp án khác nhau khi một cơ hội đang mở (chưa đóng lại) tại đúng thời điểm cuối một khoảng báo cáo đang xét. Chốt duy nhất vào "ảnh chụp có hiệu lực tại cuối khoảng" giải quyết dứt điểm mọi tổ hợp: nó tự động giữ nguyên báo cáo của các kỳ đã kết thúc (vì ảnh chụp có hiệu lực tại các thời điểm đó không đổi theo các tái phân loại xảy ra sau này), và tự động loại một cơ hội khỏi báo cáo của đúng những kỳ mà tại cuối kỳ đó nó chưa có kết quả nào được chốt.
...
| Mã AC | Bối cảnh | Hành động | Kết quả mong đợi |
| --- | --- | --- | --- |
| `AC-25.2.1` | Đồng tiền cơ sở là VND. Giai đoạn "Đàm phán" (xác suất 50%) có một cơ hội 2.000 USD và một cơ hội 20 triệu VND | Mở báo cáo dự báo doanh thu | Doanh thu dự báo của giai đoạn đã quy đổi cơ hội USD sang VND theo tỷ giá trước khi nhân xác suất và cộng dồn; không có bước nào cộng thẳng 2.000 với 20.000.000 |
...
- **`BR-26.1` (Báo cáo theo người phụ trách, hai chỉ số giá trị tách biệt):** Tổng hợp theo từng người phụ trách trong một kỳ — cơ hội đang mở xét theo ngày dự kiến đóng rơi trong kỳ, cơ hội đã đóng xét theo ảnh chụp có hiệu lực và có thời điểm đóng rơi trong kỳ (`BR-21.3`). Một cơ hội **đang mở tại thời điểm xem báo cáo nhưng đã từng có ảnh chụp kết quả được tính vào chính kỳ đang xét** (trường hợp tái phân loại: cơ hội trở lại đang mở sau khi ảnh chụp cũ đã được tính vào kỳ đó theo `BR-21.3`) **không được tính thêm một lần nữa** ở phần "đang mở" của cùng kỳ đó — mỗi cơ hội chỉ xuất hiện đúng một lần trong một báo cáo, dù dưới dạng đã đóng hay đang mở. Báo cáo hiển thị **hai chỉ số giá trị riêng biệt**, không gộp làm một:
  - **Giá trị phụ trách:** 100% giá trị của mọi cơ hội mà người đó là người phụ trách, không chia theo tỷ lệ — phản ánh khối lượng công việc đang xử lý.
  - **Doanh số ghi nhận:** giá trị đã phân bổ theo tỷ lệ chia doanh số (`FEAT-24`), chỉ tính cơ hội đã đóng Thắng — cùng cách tính và cùng con số với `BR-27.2` (hạn ngạch), để hai báo cáo luôn khớp nhau trên cùng một người trong cùng một kỳ.

  Tỷ lệ thắng tính trên số cơ hội đã đóng (Thắng chia cho tổng Thắng và Thua); cơ hội đang mở không tính vào mẫu số.

  **Lý do nghiệp vụ:** "Tổng giá trị" nếu chỉ nêu một con số duy nhất sẽ mơ hồ giữa hai câu hỏi khác nhau — "người này đang xử lý bao nhiêu tiền" và "người này được ghi công bao nhiêu tiền" — khiến báo cáo hiệu suất và báo cáo hạn ngạch cho hai số khác nhau trên cùng một người mà không ai giải thích được vì sao.
...
| `AC-26.1.3` | Nhân viên A đóng thắng một cơ hội 1 tỷ nhưng chỉ được chia 60% doanh số (`FEAT-24`) | Mở báo cáo hiệu suất người phụ trách và báo cáo hạn ngạch (`FEAT-27`) trong cùng kỳ | Giá trị phụ trách hiển thị 1 tỷ; Doanh số ghi nhận hiển thị 600 triệu — khớp đúng con số Hạn ngạch đã ghi nhận cho A trong báo cáo `FEAT-27` |
| `AC-26.1.4` | Cơ hội đóng Thắng trong kỳ, ngày dự kiến đóng ban đầu cũng rơi trong chính kỳ đó. Sau đó cơ hội được mở lại (còn đang mở khi xem báo cáo), ảnh chụp Thắng cũ vẫn được tính vào kỳ này theo `BR-21.3` | Mở báo cáo hiệu suất người phụ trách cho kỳ đó | Cơ hội chỉ xuất hiện đúng một lần, ở phần đã đóng (theo ảnh chụp Thắng); không xuất hiện thêm lần nữa ở phần đang mở dù ngày dự kiến đóng của nó vẫn rơi trong kỳ |
| `AC-26.2.1` | Có cơ hội đến từ hai Nguồn gốc chính khác nhau | Mở báo cáo theo nguồn gốc | Mỗi nguồn hiển thị đúng số lượng và 100% tổng giá trị cơ hội thuộc nguồn đó, không chia theo tỷ lệ |
...
- **`BR-38.1` (Tạo mới, không sửa cũ):** Nghiệp vụ này **tạo một cơ hội mới**, giữ nguyên cơ hội thua cũ cùng lý do thất bại của nó. Cơ hội mới mang liên kết tham chiếu tới cơ hội cũ.
  ...
  **Lý do nghiệp vụ:** Khách quay lại sau khi thua là một lần bán mới, không phải việc sửa sai một kết quả cũ — tái phân loại (`FEAT-21`) chỉ dành cho trường hợp kết quả trước đó bị **ghi sai** (ví dụ đóng thua nhầm một thương vụ thực ra vẫn còn đàm phán), không dành cho trường hợp kết quả cũ đúng nhưng nghiệp vụ mới phát sinh sau đó. Nếu dùng nhầm đường tái phân loại cho tình huống này, ảnh chụp của cơ hội thua thật (đã đóng đúng lúc đó, với đúng lý do thất bại thật) sẽ bị đánh dấu Hết hiệu lực một cách sai trái — làm mất dấu vết phân tích nguyên nhân thất bại thật (vốn là lý do tồn tại của `FEAT-19`). Nếu diễn ra trong cùng kỳ báo cáo chưa kết thúc, theo `BR-21.3` báo cáo của kỳ đó chỉ tính ảnh chụp mới nhất — kết quả Thua thật sẽ **biến mất hoàn toàn** khỏi kỳ đó, như thể thương vụ thất bại này chưa từng xảy ra, trong khi thực tế đây là hai thương vụ khác nhau đáng ra phải cùng được ghi nhận.
